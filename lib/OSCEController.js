...
class OSCEController {
    ...
    async processUserInput(input) {
        ...
        try {
            ...
            const { actionType, content } = this._extractSlashCommand(trimmedInput);
            const effectiveInput = content;

            this.logger.logUserAction('user_input', null, {
                input: effectiveInput.substring(0, 100),
            ...
            if (this.awaitingCaseSelection) {
                return await this._handleCaseSelection(effectiveInput);
            }

            if (this.showingResults) {
                return this._handleResultsMode(effectiveInput);
            }
            ...
            const specialResponse = await this._handleSpecialCommands(effectiveInput);
            if (specialResponse) {
                return specialResponse;
            }

            // Determine request type for patient simulation
            const requestType = actionType || this._categorizeInput(effectiveInput);

            // Track the action only when using explicit commands
            let triggeredItems = [];
            try {
                const trackingType = actionType || 'general';
                triggeredItems = this.performanceTracker.trackAction(effectiveInput, trackingType);
            } catch (trackingError) {
                this.logger.error(
                    'Error tracking user action',
                    { error: trackingError.message, input: effectiveInput.substring(0, 50) },
                    'osce_controller'
                );
            ...
            let patientResponse;
            try {
                patientResponse = await this.patientSimulator.respondAsPatient(effectiveInput, requestType);
...
⌨️ **Command Shortcuts:**
• Use slash commands to categorize actions
   /history  – history taking
   /exam     – physical examination
   /investigation – diagnostic tests
   /diagnosis – provide diagnosis
   /management – outline treatment
• Progress is only tracked when using these commands
...
• "exit" - Quit application`;
    ...
    _formatCaseStartMessage(caseData) {
        ...
        message += `└───────────────────────────────────────────────────────────────┘\n`;
        message += `\n⌨️  Use slash commands to categorize actions (e.g., /history, /exam, /investigation)\n`;
        message += `\n⚡ Quick Commands:\n`;
...
- Review your performance above`;
    ...
    /**
     * Extract explicit slash command for action categorization
     * @param {string} input - Raw user input
     * @returns {{actionType: (string|null), content: string}} Parsed command and remaining content
     * @private
     */
    _extractSlashCommand(input) {
        if (!input.startsWith('/')) {
            return { actionType: null, content: input };
        }
        ...
        switch (command.toLowerCase()) {
            case '/history':
            case '/hx':
                return { actionType: 'history', content };
            case '/exam':
            case '/examination':
                return { actionType: 'examination', content };
            case '/investigation':
            case '/test':
            case '/ix':
                return { actionType: 'investigation', content };
            case '/diagnosis':
            case '/dx':
                return { actionType: 'diagnosis', content };
            case '/management':
            case '/plan':
                return { actionType: 'management', content };
            default:
                return { actionType: null, content: input };
        }
    }
