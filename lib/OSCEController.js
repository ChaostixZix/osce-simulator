...
class OSCEController {
    ...
    getHelp() {
        if (!this.isActive) {
            return `OSCE Medical Training System Help:
- Type "start osce" to begin
- Select cases by typing the case ID
- Interact naturally with the AI patient
- Type "score" to see current progress
- Type "end case" to finish and get results
- Type "help" for this message`;
        }

        if (this.awaitingCaseSelection) {
            return `Case Selection Help:
- Type the case ID (e.g., "stemi-001") to select a case
- Type "list" to see available cases again
- Type "help" for this message`;
        }

        if (this.currentCase) {
            return `Active Case Help:
- Ask the patient questions about their symptoms
- Request physical examinations (e.g., "I'd like to examine your heart")
- Order tests (e.g., "I'd like to get an ECG")
- Type "score" to see your current progress
- Type "case info" to review case details
- Type "end case" to finish and get results
- Type "help" for this message`;
        }
