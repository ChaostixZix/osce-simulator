...
class CaseManagerService
{
    ...
    public function loadAvailableCases()
    {
        ...
        try {
            ...
            // Get all JSON files from the cases directory using dedicated disk
            $files = collect(Storage::disk('cases')->files('.'))
                ->map(function($path){
                    return $path; // paths are relative to cases root
                })
                ->filter(function($file){
                    return pathinfo($file, PATHINFO_EXTENSION) === 'json' && basename($file) !== 'case-schema.json';
                })
                ->map(function($file){
                    return $file; // keep relative path under cases disk
                })
                ->values()
                ->all();
            $jsonFiles = $files;
    ...
    {
        try {
            $content = Storage::disk('cases')->get($filePath);
